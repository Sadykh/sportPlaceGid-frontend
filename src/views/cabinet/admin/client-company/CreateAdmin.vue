<template>
    ...
    <div>
        ...
        <section class="content-header">
            <h1>
                Добавление администратора к компании грузовладельца
            </h1>
            <ol class="breadcrumb">
                <li>
...
<script>
    ...
    export default {
        computed: {
            ...mapState(['identity']),
            ...mapGetters({
                companyList: 'clientCompany/items',
        ...
        methods: {
            async formSubmit(e) {
                ...
                const result = await this.$store.dispatch('clientCompany/createAdmin', payloadData)
                if (result) {
                    this.$router.push({name: 'CabinetAdminClientCompanyIndex'})
                    toastr.success('Компания грузовладельца успешно создан')
                }
            }
        }
    }
