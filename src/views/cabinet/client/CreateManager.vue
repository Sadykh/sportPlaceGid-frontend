    ...
    <div>
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Добавление менеджера</li>
            </ol>
        </section>
        <!-- Main content -->
...
<script>
    ...
    export default {
        computed: {
            ...mapState(['identity']),
            ...mapGetters({
                isLoading: 'clientManager/isLoading',
                errors: 'clientManager/errors'
            }),
        },
        data() {
            return {
        ...
        methods: {
            async formSubmit(e) {
                ...
                const result = await this.$store.dispatch('clientManager/create', payloadData)
                if (result) {
                    this.$router.push({name: 'CabinetClientCarrierCompanyIndex'})
                    toastr.success('Менеджер успешно добавлен')
                }
            }
        }
    }
